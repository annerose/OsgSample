...
//	    		else
    			...
    			s_lDownTicks = System.currentTimeMillis();
    			...
    			{
    				osgNativeLib.mouseButtonPressEvent(event.getX(0), event.getY(0), 3);
    			}
...
//    			mode = moveTypes.NONE;
    		...
    		case MotionEvent.ACTION_MOVE:
    			...
    			// ±ÜÃâºÍ¶àÖ¸³åÍ»
    			if(System.currentTimeMillis() - s_lDownTicks > 200 &&  !mIsInMulTouch)
    			{
	    			...
	    			{
	    				float dx = event.getX(0) - oneFingerOrigin.x;
	    				float dy = event.getY(0) - oneFingerOrigin.y;
	    				
	    				osgNativeLib.mouseMoveEvent(dx, dy, 3);
	    			}
...
//    			else
//    			{
//    				osgNativeLib.mouseButtonReleaseEvent(event.getX(0), event.getY(0), 3);
//    			}
//    			
    			mIsInMulTouch = false;
    			break;
...
//    			osgNativeLib.mouseMoveEvent(oneFingerOrigin.x,oneFingerOrigin.y);
    			...
    			distanceOrigin = sqrDistance(event);
    			osgNativeLib.mouseButtonPressEvent(1,  0 , 2);
...
//    			}
    			...
    			float distance = sqrDistance(event);    			
    			float factor = distance / distanceOrigin;
    			osgNativeLib.mouseMoveEvent(factor, 0, 2);
...
//    			float distance = sqrDistance(event);    			
//    			float factor = distance / distanceOrigin;
//    			osgNativeLib.mouseMoveEvent(factor, 0);
    			break;
    		case MotionEvent.ACTION_UP:
//    			mode =moveTypes.NONE;
